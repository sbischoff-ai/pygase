...
initial_game_state = GameState(
    ...
    chaser=None, # id of player who is chaser
...
def time_step(game_state, dt):
    ...
    if game_state.protection:
        ...
        return {
            ...
            'protection': False if new_countdown >= 0.0 else True
        }
    # Check if chaser got someone
    for player_id, player in game_state.players.items():
        if not player_id == game_state.chaser:
            # Calculate their distance to the chaser.
            dx = player['position'][0] - game_state.chaser['position'][0]
            dy = player['position'][1] - game_state.chaser['position'][1]
